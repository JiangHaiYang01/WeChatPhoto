...
class CircleProgressView @JvmOverloads constructor(
    ...
    private val mReachPaint: Paint by lazy { Paint() } // 绘制已完成进度画笔
    private val mNormalPaint: Paint by lazy { Paint() } // 绘制未完成进度画笔
    private val mTextPaint: Paint by lazy { Paint() } // 绘制进度值字体画笔
    private val mOutPaint: Paint by lazy { Paint() } // 外部圆环画笔
    private val mInnerBackgroundPaint: Paint by lazy { Paint() } // 内部背景画笔

    private var mTextSize = sp2px(14f) // 进度值字体大小
    private var mNormalBarSize = dp2px(2f) // 未完成进度条大小
    private var mReachBarSize = dp2px(2f) // 未完成进度条大小
    private var mOuterSize = dp2px(1f) // 外层圆环宽度
    private var mTextSkewX = 0f // 进度值字体倾斜角度 = 0f
    private var mRadius = dp2px(20f) // 半径
    private var mStartArc = 0 // 起始角度 = 0
    private var mInnerPadding = dp2px(1f) // 内部圆与外部圆间距
    ...
    private var mOuterColor = Color.TRANSPARENT// 外部圆环颜色 = 0
    private var mInnerBackgroundColor = Color.TRANSPARENT// 内部背景填充颜色 = 0
    ...
    var reachBarSize: Int
        get() = mReachBarSize
        set(reachBarSize) {
            mReachBarSize = dp2px(reachBarSize.toFloat())
            invalidate()
        }
    ...
    var normalBarSize: Int
        get() = mNormalBarSize
        set(normalBarSize) {
            mNormalBarSize = dp2px(normalBarSize.toFloat())
            invalidate()
        }
    ...
    var innerPadding: Int
        get() = mInnerPadding
        set(innerPadding) {
            mInnerPadding = dp2px(innerPadding.toFloat())
    ...
    var outerSize: Int
        get() = mOuterSize
        set(outerSize) {
            mOuterSize = dp2px(outerSize.toFloat())
            invalidate()
        }
...
//            mOutPaint = Paint()
    ...
    private fun obtainAttributes(attrs: AttributeSet?) {
        val ta = context.obtainStyledAttributes(attrs, R.styleable.CustomProgressView)
        mProgressStyle =
            ta.getInt(R.styleable.CustomProgressView_custom_progressStyle, ProgressStyle.NORMAL)
        // 获取三种风格通用的属性
        mNormalBarSize = ta.getDimension(
            R.styleable.CustomProgressView_custom_progressNormalSize,
            mNormalBarSize.toFloat()
        ).toInt()
        mNormalBarColor =
            ta.getColor(R.styleable.CustomProgressView_custom_progressNormalColor, mNormalBarColor)
        mReachBarSize = ta.getDimension(
            R.styleable.CustomProgressView_custom_progressReachSize,
            mReachBarSize.toFloat()
        ).toInt()
        mReachBarColor =
            ta.getColor(R.styleable.CustomProgressView_custom_progressReachColor, mReachBarColor)
        mTextSize = ta.getDimension(
            R.styleable.CustomProgressView_custom_progressTextSize,
            mTextSize.toFloat()
        ).toInt()
        mTextColor =
            ta.getColor(R.styleable.CustomProgressView_custom_progressTextColor, mTextColor)
        mTextSkewX = ta.getDimension(R.styleable.CustomProgressView_custom_progressTextSkewX, 0f)
        if (ta.hasValue(R.styleable.CustomProgressView_custom_progressTextSuffix)) {
            mTextSuffix =
                ta.getString(R.styleable.CustomProgressView_custom_progressTextSuffix).toString()
        }
        if (ta.hasValue(R.styleable.CustomProgressView_custom_progressTextPrefix)) {
            mTextPrefix =
                ta.getString(R.styleable.CustomProgressView_custom_progressTextPrefix).toString()
        }
        mTextVisible =
            ta.getBoolean(R.styleable.CustomProgressView_custom_progressTextVisible, mTextVisible)
        mRadius =
            ta.getDimension(R.styleable.CustomProgressView_custom_radius, mRadius.toFloat()).toInt()
        rectF =
            RectF((-mRadius).toFloat(), (-mRadius).toFloat(), mRadius.toFloat(), mRadius.toFloat())
        when (mProgressStyle) {
            ProgressStyle.FILL_IN -> {
            ...
            ProgressStyle.FILL_IN_ARC -> {
                mStartArc =
                    ta.getInt(R.styleable.CustomProgressView_custom_progressStartArc, 0) + 270
                mInnerPadding = ta.getDimension(
                    R.styleable.CustomProgressView_custom_innerPadding,
                    mInnerPadding.toFloat()
                ).toInt()
                mOuterColor =
                    ta.getColor(R.styleable.CustomProgressView_custom_outerColor, mReachBarColor)
                mOuterSize = ta.getDimension(
                    R.styleable.CustomProgressView_custom_outerSize,
                    mOuterSize.toFloat()
                ).toInt()
                mReachBarSize = 0 // 将画笔大小重置为0
                mNormalBarSize = 0
                if (!ta.hasValue(R.styleable.CustomProgressView_custom_progressNormalColor)) {
            ...
            ProgressStyle.NORMAL -> {
                mReachCapRound =
                    ta.getBoolean(R.styleable.CustomProgressView_custom_reachCapRound, true)
                mStartArc =
                    ta.getInt(R.styleable.CustomProgressView_custom_progressStartArc, 0) + 270
                if (ta.hasValue(R.styleable.CustomProgressView_custom_innerBackgroundColor)) {
                    mInnerBackgroundColor = ta.getColor(
                        R.styleable.CustomProgressView_custom_innerBackgroundColor,
                        Color.argb(0, 0, 0, 0)
                    )
    ...
    override fun onRestoreInstanceState(state: Parcelable) {
        if (state is Bundle) {
            ...
            mTextSuffix = bundle.getString(TEXT_SUFFIX).toString()
            mTextPrefix = bundle.getString(TEXT_PREFIX).toString()
